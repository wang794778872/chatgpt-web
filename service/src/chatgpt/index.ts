...
(async () => {
	...
	api4 = await initGPTAPIEx(process.env.OPENAI_API_KEY, 'gpt-4')
})()
...
async function chatReplyProcessEx(model: string, options: RequestOptions) {
	...
	try {
		const get_apikey = await get_openai_apikey(false, model)
		let apiEx: ChatGPTAPI | ChatGPTUnofficialProxyAPI
		...
		if ('401' == code) {
			const change_apikey = await get_openai_apikey(true, model)
