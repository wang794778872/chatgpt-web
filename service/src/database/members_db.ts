...
async function before_user_chat(username: string, model: string) {
	if (!username) {     //非会员访问
	...
	if ("gpt-4" == model) {
		try {
			if (results.gpt4_everyday_count == 0 || results.gpt4_everyday_count == undefined)
				return { status: false, message: "不支持GPT4", data: undefined }
			if (Math.floor(results.gpt4_last_usetime) != Math.floor((new Date().getTime()) / 86400)) {
				//今日首次使用
				results.gpt4_last_usetime = Math.floor((new Date().getTime()) / 86400)
				results.gpt4_today_count = 0;
			}
			console.log("fffff  ", results.gpt4_today_count, results.gpt4_everyday_count, results.gpt4_last_usetime)
			if (results.gpt4_today_count < results.gpt4_everyday_count) {
				results.gpt4_today_count += 1
				//支持gpt
			}
			else {
				return { status: false, message: "今日的GPT4额度已用尽，请等待明日再使用", data: undefined }
			}
		}
		catch (error) {
			return { status: false, message: "不支持GPT4", data: undefined }
	...
	if (results.stop_time == 9999)  //永久会员
		return { status: true, message: "永久会员", data: results }
	else if (results.stop_time > new Date().getTime()) {
		return { status: true, message: "会员未过期", data: results }
	}
...
async function user_register_new_with_code(username: string, passwd: string, code_info: any) {
	const new_member = {
		...
		stop_time: 0,
		avail_num: 0,
		gpt4_everyday_count: 0,
		gpt4_today_count: 0,
		gpt4_last_usetime: Math.floor((new Date().getTime()) / 86400),
	}
	...
	new_member.gpt4_everyday_count = code_info.gpt4_everyday_count
...
async function user_code_active(username: string, code_info: any) {
	...
	if ("gpt4_everyday_count" in code_info) {
		if ("gpt4_everyday_count" in results) {
			results.gpt4_everyday_count += code_info.gpt4_everyday_count
		}
		else {
			results.gpt4_everyday_count = code_info.gpt4_everyday_count
		}
